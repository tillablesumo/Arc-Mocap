        ...
        if (receive_result) {
            if (*receive_result) {
                ...
                if (!ekf_paused_map[node_id]) {
                    ...
                    // 显示VR控制数据
                    std::cout << "=== VR Control Data ===" << std::endl;
                    std::cout << std::format("Buttons: 0x{:x}", raw_packet.buttons) << std::endl;
                    std::cout << std::format("Joystick: [{}, {}]", 
                                         std::fixed << std::setprecision(3) << raw_packet.joystick_x, 
                                         raw_packet.joystick_y) << std::endl;
                    
                    // 构建并发送SlimeVR格式数据包（包含VR控制数据）
                    SlimeVRPacket slimevr_packet;
                    slimevr_packet.node_id = node_id;
                    slimevr_packet.qx = corrected_q.x;
                    slimevr_packet.qy = corrected_q.y;
                    slimevr_packet.qz = corrected_q.z;
                    slimevr_packet.qw = corrected_q.w;
                    slimevr_packet.buttons = raw_packet.buttons;
                    slimevr_packet.joystick_x = raw_packet.joystick_x;
                    slimevr_packet.joystick_y = raw_packet.joystick_y;
                    
                    // 发送SlimeVR数据包到本地SlimeVR服务器
                    sockaddr_in slimevr_addr;
                    slimevr_addr.sin_family = AF_INET;
                    slimevr_addr.sin_addr.s_addr = inet_addr("127.0.0.1"); // 本地地址
                    slimevr_addr.sin_port = htons(6969); // SlimeVR默认端口
                    
                    int bytes_sent = sendto(*slimevr_socket, (char*)&slimevr_packet, sizeof(SlimeVRPacket), 0, 
                                          (sockaddr*)&slimevr_addr, sizeof(slimevr_addr));
                    if (bytes_sent == SOCKET_ERROR) {
                        std::cerr << std::format("Failed to send SlimeVR packet: {}", WSAGetLastError()) << std::endl;
                    }
